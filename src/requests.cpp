...
#define OP_FIELD_OFFSET(offset)        \
    int loc_bitfields_offset = offset; \
    int curr = loc_bitfields_offset + message[loc_bitfields_offset] + 1;

#define NEW_FIELD(name, loc_offset) \
    exec_details.name = decode_field_##name(message, loc_offset);
#include "execution_order_fields.inl"

#define NEW_OP_FIELD(name, byte, bit, length)                      \
    if (1 & (message[loc_bitfields_offset + byte] >> (bit - 1))) { \
        exec_details.name = decode_field_##name(message, curr);    \
        curr += length;                                            \
    }
...
#define NEW_FIELD(name, loc_offset) \
    restatement_details.name = decode_field_##name(message, loc_offset);
#include "restatement_order_fields.inl"

#define NEW_OP_FIELD(name, byte, bit, length)                          \
    if (1 & (message[loc_bitfields_offset + byte] >> (bit - 1))) {     \
        restatement_details.name = decode_field_##name(message, curr); \
        curr += length;                                                \
    }
...
std::vector<unsigned char> request_optional_fields_for_message(const ResponseType t)
{
    std::vector<unsigned char> res;
    int size = 0, idx = -1;

#define OP_FIELD_FOR_MESSAGE(order_type, byte, bit) \
    if (t == ResponseType::order_type) {            \
        size = std::max(size, byte);                \
    }

#include "current_op_fields_for_messages.inl"
